...
public class MeasureToolbar: UIToolbar, AGSGeoViewTouchDelegate {
    ...
    public let areaFreeSketchEditor = AGSSketchEditor()
    ...
    public var selectedFreeAreaUnit: AGSAreaUnit = {
        if NSLocale.current.usesMetricSystem {
            return AGSAreaUnit(unitID: AGSAreaUnitID.hectares) ?? AGSAreaUnit.squareKilometers()
        } else {
            return AGSAreaUnit(unitID: AGSAreaUnitID.acres) ?? AGSAreaUnit.squareMiles()
    ...
    public var segControl: UISegmentedControl!
    ...
    override public func layoutSubviews() {
        switch mode {
        case .length, .area, .freehandpolygon:
    ...
    private func startFreeHandPolygonMode() {
        guard mode != MeasureToolbarMode.freehandpolygon else {
            return
        }
        
        mode = .freehandpolygon
        selectionOverlay?.isVisible = false
        mapView?.sketchEditor = areaFreeSketchEditor
        
        if !areaFreeSketchEditor.isStarted {
            areaFreeSketchEditor.start(with: AGSSketchCreationMode.freehandPolygon)
        }
    ...
    private lazy var linearUnits: [AGSLinearUnit] = {
        let linearUnitIDs: [AGSLinearUnitID] = [.inches, .kilometers, .meters, .miles]
    ...
    private lazy var areaUnits: [AGSAreaUnit] = {
        let areaUnitIDs: [AGSAreaUnitID] = [.hectares, .squareKilometers, .squareMeters, .squareMiles]
        return areaUnitIDs
            .compactMap(AGSAreaUnit.init)
            .sorted { $0.pluralDisplayName < $1.pluralDisplayName }
    }()
    
    private lazy var areaFreeUnits: [AGSAreaUnit] = {
        let areaFreeUnitIDs: [AGSAreaUnitID] = [.hectares, .squareKilometers, .squareMeters, .squareMiles]
        return areaFreeUnitIDs
            .compactMap(AGSAreaUnit.init)
            .sorted { $0.pluralDisplayName < $1.pluralDisplayName }
    }()
    ...
    private func sketchEditorGeometryDidChange(_ notification: Notification) {
        guard let sketchEditor = notification.object as? AGSSketchEditor,
            sketchEditor == lineSketchEditor || sketchEditor == areaSketchEditor  || sketchEditor == areaFreeSketchEditor else {
                return
        }
        updateMeasurement()
    }
    ...
    private func calculateSketchFreeArea() -> Double {
        guard mapView?.sketchEditor?.isSketchValid == true, let geom = mapView?.sketchEditor?.geometry else {
            return 0
        }
        
        return calculateFreeArea(of: geom)
    }
    
    private func calculateFreeArea(of geom: AGSGeometry) -> Double {
        // if planar is very large then just return that, geodetic might take too long
        if let linearUnit = geom.spatialReference?.unit as? AGSLinearUnit {
            let planar = AGSGeometryEngine.area(of: geom)
            if let planarMiles = linearUnit.toAreaUnit()?.convert(planar, to: AGSAreaUnit.squareMiles()),
